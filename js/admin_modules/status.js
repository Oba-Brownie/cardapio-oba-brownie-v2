...
function atualizarBotaoStatus(estaAberta) {
    ...
    if (estaAberta) {
        btn.innerHTML = "LOJA ABERTA"; btn.className = "btn-status-on";
    } else {
        btn.innerHTML = "LOJA FECHADA"; btn.className = "btn-status-off";
    }
}
...
export async function carregarStatusInicial() {
    ...
    if (data) atualizarBotaoStatus(data.loja_aberta);
    else atualizarBotaoStatus(false);
}
